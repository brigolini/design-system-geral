...
export async function loader({ params }: LoaderFunctionArgs) {
  ...
  const pessoa = pessoaService.getById(params.id!);
  const cidadeNames: Record<string, string> = {};
  for (const e of pessoa.enderecos) {
    if (!cidadeNames[e.cidadeId]) {
      try {
        cidadeNames[e.cidadeId] = cidadeService.getById(e.cidadeId).nome;
      } catch {
        cidadeNames[e.cidadeId] = '';
      }
    }
  }
  return Response.json({ pessoa, cidadeNames });
}
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  try {
    ...
    pessoaService.update(params.id!, input);
    return redirect('/pessoas');
  } catch {
    return Response.json({ error: 'Erro ao atualizar pessoa' }, { status: 400 });
  }
}
...
export default function PessoaEdit() {
  const { pessoa, cidadeNames } = useLoaderData() as { pessoa: PessoaData; cidadeNames: Record<string, string> };
          ...
          {fields.map((field, index) => (
            <div key={field.id} style={{ border: '1px solid var(--color-almg-brand-secondary)', borderRadius: '8px', padding: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 200px', gap: '12px', marginTop: '12px' }}>
                <AlmgAsyncAutocomplete name={`enderecos.${index}.cidadeId`} form={form} label="Cidade" fetchOptions={fetchCidades} placeholder="Buscar cidade..." required initialDisplayValue={cidadeNames[pessoa.enderecos[index]?.cidadeId] ?? ''} />
